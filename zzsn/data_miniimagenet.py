...
from zzsn.constants import MINIIMAGENET_DIR, MINIIMAGENET_SPLITS_DIR, MINIIMAGENET_CLASSES \
...
class MiniImageNetDataset(Dataset):
    def __init__(
        ...
        self.classes = np.unique(split["label"].to_numpy(dtype=str)) #= np.array([str(el) for el in split["label"].to_numpy()])
        self.n_support: int = n_support
        self.n_query: int = n_query
        self.data_dir: str = data_dir

    def __len__(self) -> int:
        return self.classes.size #len(self.classes)
...
def create_data_loader_miniimage(
    split: str,
    n_support: int,
    n_query: int,
    n_way: int,
